...
function ManageInterviewer() {
    ...
    const handleBlockClick = (id) => {
      const values = {};
      axios
        .put(`/api/admin/manage/interviewer/status/${id}`,values, {
          headers: {
            authToken: localStorage.getItem("admintoken"),
          },
        })
        .then((res) => {
          setUsers(res.data.interviewers);
          Toast.fire({
            icon: "success",
    ...
    const handleEditClick = (id) => {
      navigate('/admin/interviewer/update', {state: {id : id}})
    }
  
    useEffect(() => {
      axios
        .get(`/api/admin/manage/interviewer`, {
          headers: {
            authToken: localStorage.getItem("admintoken"),
          },
        })
